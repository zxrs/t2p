...
mod sys;
use sys::{
    t2p_free, t2p_init, t2p_write_pdf, tdata_t, thandle_t, tmsize_t, TIFFClientOpen, TIFFClose,
    T2P, TIFF,
};
...
pub fn generate_pdf(tiff: &[u8]) -> Result<Vec<u8>> {
    let t2p = unsafe { t2p_init() };
    ensure!(!t2p.is_null(), "failed to init t2p");
    ...
    ensure!(!input_tiff.is_null(), "failed to open input.");
    ...
    ensure!(!output_tiff.is_null(), "failed to open output.");
    ...
    ensure!(ret > 0 && t2p.0.t2p_error != 1, "failed to write pdf.");
